...
<section class="container clearfix">
 ...
 <div class="mod">    
     <table class="datagrid">
        ...
        <tbody>
        <c:if test="${empty userCourses}">您还没有创建课程</c:if>
        <c:if test="${!empty userCourses}">
        <c:forEach items="${userCourses}" var="fa">
           <tr>
            <td class="favorite-name">
             <a href="courseDetailPage.htm?courseId=${fa.course.courseId}">${fa.course.courseTitle}</a>
            </td>
            <td class="favorite-url"></td>
            <td class="favorite-url">
              ${fn:substring(fa.course.courseIntro,0,40)}         
            </td>
            
            <td class="tac" style="text-align: left;">
              ${fa.course.courseState}
            </td>          
            <td class="tac">
            <input type="hidden" value="${fa.course.courseId}" class="cid">
            <c:if test="${fa.course.courseState=='批准'}">
