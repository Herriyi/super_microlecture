  ...
  <div class="container">
    ...
    <div class="user-status-bar">
      ...
      <div class="status-item-wrapper">
        ...
        <c:if test="${sumMail!=0}">
        <a href="goPrivateMail.htm" class="remind-number" id="message-remind-number" style="display:block;">${sumMail}</a>
        </c:if>
      </div>
      <div class="status-item-wrapper" id="user-nav-item-wrapper">
        <a href="goPersonnal.htm?userId=${user.userId}" class="status-item user-nav-item" id="user"><img src="<c:url value="${user.headImage.imageSmall}"/>" alt="fanfanle" class="avatar" /><span class="nickname">${userName}</span></a>
        <div class="user-nav-menus">
        <a href="goPersonnal.htm?userId=${user.userId}">个人主页</a>
