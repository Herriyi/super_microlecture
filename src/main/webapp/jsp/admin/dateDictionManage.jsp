...
$(function(){
   $("#saveIterm").click(function(){
	...
	$("a.btn.alter").click(function(){
          var key=$(this).parent().parent().children("td").eq(0).html();
          var value=$(this).parent().parent().children("td").eq(3).html();
          var intro=$(this).parent().parent().children("td").eq(6).html();
          var id=$(this).parent().parent().children("td").eq(7).children("input.dataId").val();
          //alert(key+"||"+value+"||"+intro+"||"+id);
          $("#alterKey").val(key);
          $("#alterValue").val(value);
          $("#alterIntro").val(intro);
          $("#itermIdTemp").val(id);
		});
	...
	$("button.btn.btn-danger").click(function(){
         if(confirm("是否真的要删除？")){
        	 var id=$(this).parent().parent().children("td").eq(7).children("input.dataId").val();
        	 //alert(id);
        	 location.href="deletIterm.htm?itermId="+id;
             }else{
               return false;
                 }
		});
		...
		<div class="page-container">
				...
				<div class="row">
					<div class="span12"><a href="#newTerm" data-toggle="modal" class="btn pull-right">添加词条</a>					
						...
						<table class="table table-striped sortable">
							...
							<tbody>
							<c:forEach items="${dataDictionaries}" var="dictionaries">
								<tr>
								<td style="text-align:center;">${dictionaries.dicKey}</td>
								<td style="text-align:center;">&nbsp;</td>
								<td>&nbsp;</td>
								<td style="text-align:center;">${dictionaries.dicValue}</td>
								<td>&nbsp;</td>
								<td>&nbsp;</td>
								<td style="text-align:center;">${dictionaries.intro}</td>
								<td><input type="hidden" class="dataId" value="${dictionaries.dicId}"></td>
								<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>	
								<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
								<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
								<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>	
			...
			<!-- ------------------------添加词条------------------------------------ -->
				...
				<div class="modal-body">
					<form class="form-horizontal" id="form1" action="addIterm.htm" method="post">					
					<div class="control-group">
						<label for="inputKey" class="control-label">键 ：</label>
						<div class="controls">
							<input id="inputKey" name="inputKey" type="text" placeholder="请输入键" />
						</div>
					</div>
					<div class="control-group">
						<label for="inputValue" class="control-label">值：</label>
						<div class="controls">	
							<input id="inputValue" name="inputValue" type="text" placeholder="请输入值" />
						    </div>
					</div>
					<div class="control-group">
						<label for="inputIntro" class="control-label">说明：</label>
						<div class="controls">	
							<input id="inputIntro" name="inputIntro" type="text" placeholder="请输入说明" />
						    </div>					
					</div>
					</form>
				</div>
			...
			<!-- -------------------修改词条 ---------------------------- -->
			 <div id="alterTerm" class="modal hide fade">
				...
				<div class="modal-body">
					<form class="form-horizontal" id="form2" action="alterIterm.htm" method="post">
					<input type="hidden" name="itermId" id="itermIdTemp"/>
					<div class="control-group">
						<label for="inputKey" class="control-label">键 ：</label>
						<div class="controls">
							<input id="alterKey" name="alterKey" type="text" />
						</div>
					</div>
					<div class="control-group">
						<label for="alterValue" class="control-label">值：</label>
						<div class="controls">	
							<input id="alterValue" name="alterValue" type="text"/>
						    </div>
					</div>
					<div class="control-group">
						<label for="alterIntro" class="control-label">说明：</label>
						<div class="controls">	
							<input id="alterIntro" name="alterIntro" type="text"/>
						    </div>					
					</div>
					</form>
				</div>
